...
- **核心**（`core/`）：数据加载（含内置回退）、满级战法模型、红度增伤/减伤与自由属性点、8 回合蒙特卡洛战斗引擎、规则评分、战法配装、两阶段推荐搜索和本地账号存档。
...
- **推荐搜索**：阶段一枚举 C(138,3)（统御≤20）规则分预筛取 Top 200；阶段二配装 + 并行蒙特卡洛，按综合分取 Top-N。账号推荐会先限制在账号拥有集合，并使用每名武将的红度增伤/减伤参数。
